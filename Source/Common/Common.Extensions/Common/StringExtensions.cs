...
namespace Auxilia.Extensions
{
    ...
    public static class StringExtensions
    {
        ...
        /// <summary>
        /// Throws a <see cref="ArgumentException"/> when the given <see cref="string"/> is <see langword="null"/> or white space.
        /// </summary>
        /// <param name="s">String to check.</param>
        /// <param name="parameterName">Name of the parameter used in the exception.</param>
        /// <returns>The given string.</returns>
        /// <exception cref="ArgumentException">Thrown when the given string is <see langword="null"/> or white space.</exception>
        public static string ThrowIfNullOrWhiteSpace(this string s, string parameterName)
        {
            return string.IsNullOrWhiteSpace(s)
                ? throw new ArgumentException("Cannot be null or white space.", parameterName)
                : s;
        }
...
#pragma warning restore ExceptionNotDocumented // Exception is not documented
        ...
        public static bool Equals(this string s, string value, bool ignoreCase)
        {
	        return ignoreCase
		        ? StringComparer.OrdinalIgnoreCase.Equals(s, value)
		        : StringComparer.CurrentCulture.Equals(s, value);
        }
...
#pragma warning restore ExceptionNotDocumented // Exception is not documented
        ...
        public static string RemoveLead(this string s, string textToRemove, bool ignoreCase = false)
        {
            s.ThrowIfNull(nameof(s));
            textToRemove.ThrowIfNullOrEmpty(nameof(textToRemove));

            bool removeLead = ignoreCase
                ? s.StartsWith(textToRemove, StringComparison.OrdinalIgnoreCase)
                : s.StartsWith(textToRemove);

            return removeLead
                ? s.Right(s.Length - textToRemove.Length) 
                : s;
        }
        public static string RemoveTail(this string s, string textToRemove, bool ignoreCase = false)
        {
            s.ThrowIfNull(nameof(s));
            textToRemove.ThrowIfNullOrEmpty(nameof(textToRemove));

            bool removeTail = ignoreCase
                ? s.EndsWith(textToRemove, StringComparison.OrdinalIgnoreCase)
                : s.EndsWith(textToRemove);

            return removeTail
                ? s.Left(s.Length - textToRemove.Length)
                : s;
        }
